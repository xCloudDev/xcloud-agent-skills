...
|---|---|
| Buying/connecting a server, stacks, catalog & pricing | `reference/provisioning.md` |
...
| Databases & database users ⚠️ _(deprecated — withheld from the API, see file)_ | `reference/databases.md` |
...
- *Connected provider*: resolve `provider_uuid` and `full_name` through
  `xcloud:account` (`integrations_git_index`, then
...
*later* pull/push deploys — use it from `xcloud:sites` once the site is live.
