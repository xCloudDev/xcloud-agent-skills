...
- `references/shared/mcp.md` — **prefer tools from the MCP server named `xcloud`
...
Brand every user-facing reply (see `references/shared/conventions.md` → **Response
...
banner (see `references/shared/conventions.md` → **Startup banner**) — once per
conversation.
...
|---|---|
| Repository sync gate (uncommitted files, remote, push, private-repo access) | `references/domain/repo-sync.md` |
| Secrets contract (what may be forwarded vs. what must not) | `references/domain/secrets.md` |
| `nodejs` vs `lovable` site-type rule | `references/domain/site-type.md` |
...
1. **Repository sync gate** — see `references/domain/repo-sync.md`. Identify the
   ...
   transfer.
2. **Secrets check** — see `references/domain/secrets.md`. Ask for required env values
...
8. **Verify** the external HTTPS URL actually responds, then hand the site off
   — further changes (domains, SSL, cache, backups) go through the `sites` skill
   / the `ssl` skill.
...
|---|---|---|
| Analyse a repository (side-effect-free) | `POST /git/detect` | this skill calls it; endpoint lives under the `servers` skill |
| Detect-then-deploy in one call | `POST /servers/{uuid}/sites/git/auto` | the `servers` skill |
| Deploy with explicit container config | `POST /servers/{uuid}/sites/git/docker` | the `servers` skill |
| List eligible servers | `GET /servers` | the `servers` skill |
| Poll deployment status | `GET /sites/{uuid}/status` | the `sites` skill |

**Not here:** site settings after go-live, domains, cache, deployment logs →
the `sites` skill; certificates → the `ssl` skill; WordPress app management →
the `wordpress` skill; server infrastructure, firewall, PHP versions →
the `servers` skill.
...
- No local `.env` value is transmitted, logged, or committed without explicit
  per-value approval (`references/domain/secrets.md`).
...
- A public HTTPS repo or one on a connected provider needs no extra setup in
  ...
  first (`references/domain/repo-sync.md`).
- `lovable` is not a special deployment path — it is `nodejs` minus xCloud's
  provisioned database (`references/domain/site-type.md`). Don't Dockerize a project
