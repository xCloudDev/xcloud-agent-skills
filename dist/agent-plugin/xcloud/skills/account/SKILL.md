...
**Not here:** server management → the `servers` skill; site management →
the `sites` skill; deploying → the `deploy` skill; plans and invoices →
the `billing` skill.
...
every call of that task — see `references/shared/conventions.md` → **Teams**. One team
listed while the user expects more means the connection was authorized for one
team: explain how to re-authorize it with more (`references/shared/mcp.md`).
