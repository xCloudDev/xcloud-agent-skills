...
- `reference/mcp.md` — **prefer `mcp__xcloud__*` tools
...
|---|---|
| Repository sync gate (uncommitted files, remote, push, private-repo access) | `reference/deploy-app-repo-sync.md` |
| Secrets contract (what may be forwarded vs. what must not) | `reference/deploy-app-secrets.md` |
| `nodejs` vs `lovable` site-type rule | `reference/deploy-app-site-type.md` |
...
1. **Repository sync gate** — see `reference/deploy-app-repo-sync.md`. Identify the
   ...
   transfer.
2. **Secrets check** — see `reference/deploy-app-secrets.md`. Ask for required env values
...
- No local `.env` value is transmitted, logged, or committed without explicit
  per-value approval (`reference/deploy-app-secrets.md`).
...
- A public HTTPS repo or one on a connected provider needs no extra setup in
  ...
  first (`reference/deploy-app-repo-sync.md`).
- `lovable` is not a special deployment path — it is `nodejs` minus xCloud's
  provisioned database (`reference/deploy-app-site-type.md`). Don't Dockerize a project
